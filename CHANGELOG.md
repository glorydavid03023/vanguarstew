...
- Repo score spread (`benchmark/repo_score_spread.py`): a **skipped (zero-task) repo's placeholder
  ...
  for the horizon in `per_repo` with `tasks == 0` and a `_mean([])` default of `0.0`, and excludes it
  ...
  with the artifact's own. `_repo_scores` now gates on `tasks > 0` via a `_scored_repo` helper
  mirroring `weight_integrity`, so the placeholder is never read as a score; the `--generalization`
  path inherits the gate per partition (#1628).
